...
//            微众支付跳转
            if ($res['wx_bank'] == "1") {
                if ((int)$res['status'] == 1) {
                    if ($order_id) $url = $http . "://" . $_SERVER['HTTP_HOST'] . "/index.php?g=Pay&m=Barcode&a=two_wz_pay&id=" . $id . "&price=" . $price . "&checker_id=" . $checker_id . "&order_id=" . $order_id;//双屏端扫码支付收款
                    else if ($price) $url = $http . "://" . $_SERVER['HTTP_HOST'] . "/index.php?g=Pay&m=Barcode&a=wz_pay&id=" . $id . "&price=" . $price . "&checker_id=" . $checker_id;//手机端扫码支付收款
...
//        }
    ...
    public function screen_wz_alipay()
    {
        ...
        $pay_info = M("pay")->where(array("remark" => $order_info['order_sn']))->find();
        if ($pay_info) {
            $data = array(
            ...
            M("pay")->where(array("remark" => $order_info['order_sn']))->save(array("paystyle_id" => 1));
        } else {
            $wzcost_rate = M("merchants_upwz")->where("mid=" . $res['merchant_id'])->getField("aliCostRate");
            $data = array(
                "merchant_id" => $res['merchant_id'],
        ...
        M("pay")->where(array("remark" => $order_info['order_sn']))->save(array("new_order_sn" => $data['remark']));
        //构造要请求的参数数组,无需改动
        $parameter = array(
            'wbMerchantId' => $res['alipay_partner'] ? $res['alipay_partner'] : \AlipayConfig::MERCHANTID,
    ...
    public function wzali_notify_return()
    {
        ...
        if ($result['tag']) {
            $rs = M('pay')->where(array("remark" => $out_trade_no))->save(array("status" => "1", "wz_remark" => $result['tradeNo']));
...
//                M("pay")->where("remark='$remark'")->save(array("customer_id" => $openid, "new_order_sn" => $order_sn));
//                $price = $pay['price'];
...
//       支付订单提交的数据交互
    ...
    public function wx_pay_back($remark)
    {
        ...
        $wzPay = new \Wzcommon();
        $pay = M("pay")->where("remark='$remark' And status = 1")->find();
        ...
        if ($returnData == "OK") {
            ...
            if (M("pay")->where("remark='$remark'")->find()) {
                M("pay")->where("remark='$remark'")->save(array("status" => 3, "back_status" => 0));
            }
    ...
    {
        if (M("pay")->where("remark='$remark'")->find()) {
            M("pay")->where("remark='$remark'")->save(array("status" => 2, "back_status" => 1, "price_back" => $refund_amount));
        }
    }
    ...
    public function ali_pay_back($remark)
    {
        ...
        $pay = M("pay")->where(array("remark" => $remark))->find();
        ...
        if ($res['flag']) {
            ...
            M("pay")->where(array("remark" => $remark))->save(array("status" => "3"));
            return array("code" => "success", "msg" => "成功", "data" => "退款失败");
        }
...
//    支付成功  卡券信息更新
    ...
    {
        $remark = I("remark");
        $pay_one = M("pay")->where("remark='$remark' ")->find();
        if ($pay_one) {
...
//            $redirect_uri = U('Pay/Barcode/qr_weixipay', '', '', true);'http://' . $_SERVER['HTTP_HOST']
     ...
     * 刷卡支付
     */
    public function wz_micropay($id, $price, $auth_code, $checker_id)
    {
...
//        $json_str = '{"type":"FILE","data":"{\"corporationId\":\"99996\",\"app_id\":\"P5840001\",\"token\":\"9I6Dxosn\",\"file_id\":\"90000000149015678725301932931174000000014894655872533537\",\"file_hash\":\"34eb94dad72385484760cbf62f551839\",\"work_date\":\"20170313\"}"}';
    ...
    private function _getXmlSSLCurl($url, $second = 30)
    {
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, '/nasdata/www/hzsj/store/simplewind/Core/Library/Vendor/Bbshop/youngPort/cert/apiclient_cert.pem');
        //默认格式为PEM，可以注释
        curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
        curl_setopt($ch, CURLOPT_SSLKEY, '/nasdata/www/hzsj/store/simplewind/Core/Library/Vendor/Bbshop/youngPort/cert/apiclient_key.pem');
        $data = curl_exec($ch);
        //返回结果
        if ($data) {
            curl_close($ch);
