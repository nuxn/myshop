...
class MyRedis
{
    ...
    {
        $this->host = $config['host'] ? $config['host'] : "r-wz9ce005329c24f4.redis.rds.aliyuncs.com";
        $this->port = 6379;
        //$this->pwd = $config['pwd'];
        $this->pwd = $config['pwd'] ? $config['pwd'] : "q3heri6lNro2ie9wo8fn5fs7dk";
        $this->myRedis = new \Redis();
        if ($this->myRedis->connect($this->host, $this->port) == false) {
            die($this->myRedis->getLastError());
        }

        if ($this->myRedis->auth($this->pwd) == false) {
            die($this->myRedis->getLastError());
        }
    }
