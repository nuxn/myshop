...
///      @3 支付成功 而且不用输入密码
        if($result['result']['errno'] === '0'){
            ...
            $map['customer_id'] = $result['sub_openid'];
            $map['wz_remark'] =$result['orderid'];
            if($pay_change->where(array('remark'=>"$remark"))->find())$pay_change->where(array('remark'=>"$remark"))->save($map);
            return array('flag' => true, 'message' => "支付成功","openid"=>$result['sub_openid']);
...
//                return array('flag'=>true,'message'=>$queryResult);
                ...
                $map['customer_id'] = $queryResult['sub_openid'];
                $map['wz_remark'] =$queryResult['orderid'];
                if($pay_change->where(array('remark'=>"$remark"))->find())$pay_change->where(array('remark'=>"$remark"))->save($map);

                return array('flag'=>true,'message'=>'收款成功',"openid"=>$queryResult['sub_openid']);
            }else {
                //订单交易失败
        ...
        file_put_contents('./data/log/wz/weixin/shuaka.log', date("Y-m-d H:i:s") . '   ' . "时间过长2222" . PHP_EOL, FILE_APPEND | LOCK_EX);
        //return array('flag'=>false,'message'=>'收款失败，请重新收款');
...
file_put_contents('./data/log/wz/weixin/shuaka.log', date("Y-m-d H:i:s") . '   ' ."msg". json_encode($this->memessage) . PHP_EOL, FILE_APPEND | LOCK_EX);
