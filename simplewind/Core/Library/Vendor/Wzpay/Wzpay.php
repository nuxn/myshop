...
//        echo "【result】 " . $result_ . "</br>";
    ...
    private function postXmlSSLCurl($xml, $url, $second = 30)
    {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        $data = curl_exec($ch);
        file_put_contents(get_date_dir($this->wx_path) . date("Y_m_d_") . 'curl.log', date("Y-m-d H:i:s") . '当前服务器IP ' . $_SERVER['SERVER_ADDR'] . PHP_EOL, FILE_APPEND | LOCK_EX);
    ...
    public function postarrayCurl($arr, $url, $second = 30)
    {
        $ch = curl_init();
        //超时时间
        curl_setopt($ch, CURLOPT_TIMEOUT, $second);
    ...
    public function notify()
    {
        ...
        if ($sign === $data_sign && $data['status'] === '0' && $data['result_code'] === '0') {
            ...
            file_put_contents(get_date_dir($this->wx_path) . date("Y_m_d_") . 'error.log', date("Y-m-d H:i:s") . '支付回调信息有误' . $data['out_trade_no'] . PHP_EOL . PHP_EOL, FILE_APPEND | LOCK_EX);
            $result = $data;
//            $result = false;
        }
//        // 返回状态给微信服务器
...
//        $this->setParameter('mch_id', \WzPayConf_pub::MCHID);
        ...
        file_put_contents('./data/log/test.log', date("Y-m-d H:i:s") . '-WZ-PATAMS:' . json_encode($this->parameters) . PHP_EOL, FILE_APPEND | LOCK_EX);
        ...
        file_put_contents('./data/log/test.log', date("Y-m-d H:i:s") . '-WZ-RETURN:' . json_encode($result) . PHP_EOL, FILE_APPEND | LOCK_EX);
