...
class UnifiedOrder_pub extends Wxpay_client_pub
{
    ...
    function getPrepayId()
    {
        $this->postXml();
        $this->result = $this->xmlToArray($this->response);
        file_put_contents('./data/log/szlz/weixin/' . date("Y_m_") . 'wxpay_wxjs.log', date("Y-m-d H:i:s") . 'RETURN:'.json_encode($this->result) . PHP_EOL . PHP_EOL, FILE_APPEND | LOCK_EX);
...
class WxPayMicroPay extends Wxpay_client_pub
{
    ...
    public function pay()
    {
        $sub_mchid = $this->parameters['sub_mch_id'];
        $this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR'];//终端ip
        ...
        $this->result = $this->xmlToArray($this->response);
        $result = $this->result;
        file_put_contents('./data/log/szlz/weixin/' . date("Y_m_") . 'wxpay_micro.log', date("Y-m-d H:i:s") . 'result' . json_encode($result) . PHP_EOL, FILE_APPEND | LOCK_EX);
...
//        }
        ...
        //②、接口调用成功，明确返回调用失败
        if ($result["return_code"] == "FAIL" && $result["err_code"] != "USERPAYING" && $result["err_code"] != "SYSTEMERROR") {
            return array('flag' => false, 'msg' => $result["return_msg"]);
        }
        $pay_change = M("pay");
        ...
        while ($queryTimes < 20) {
            $succResult = 0;
            $queryTimes += 2;
            $queryResult = $this->query($out_trade_no, $succResult, $sub_mchid);
            //如果需要等待1s后继续
            if ($succResult == 2) {
                sleep(2);
                continue;
        ...
        //④、次确认失败，则撤销订单
        if (!$this->cancel($out_trade_no, $sub_mchid)) {
...
//            throw new WxpayException("撤销单失败！");
     ...
     */
    public function query($out_trade_no, &$succCode, $sub_mchid)
    {
        $this->parameters = null;
        $this->parameters["out_trade_no"] = $out_trade_no;
        $this->parameters["sub_mch_id"] = $sub_mchid;
        ...
        $this->result = $this->xmlToArray($this->response);
        $result = $this->result;
        file_put_contents('./data/log/szlz/weixin/' . date("Y_m_") . 'wxpay_micro.log', date("Y-m-d H:i:s") . '-QUERY：' . json_encode($result) . PHP_EOL . PHP_EOL, FILE_APPEND | LOCK_EX);
     ...
     * @return bool
     */
    public function cancel($out_trade_no, $mchid, $depth = 0)
    {
        if ($depth > 5) {
        ...
        $this->parameters = null;
        $this->parameters["out_trade_no"] = $out_trade_no;
        $this->parameters["sub_mch_id"] = $mchid;
        ...
        $this->result = $this->xmlToArray($this->response);
        $result = $this->result;
        file_put_contents('./data/log/szlz/weixin/' . date("Y_m_") . 'wxpay_micro.log', date("Y-m-d H:i:s") . '-CANCEL：' . json_encode($result) . PHP_EOL . PHP_EOL, FILE_APPEND | LOCK_EX);
