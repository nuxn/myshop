...
//        echo "【result】 " . $result_ . "</br>";
    ...
    {
        file_put_contents('./data/log/wz/weixin/'. date("Y_m_") .'curl.log', date("Y-m-d H:i:s").'curl请求url' . $url. PHP_EOL, FILE_APPEND | LOCK_EX);
        file_put_contents('./data/log/wz/weixin/'. date("Y_m_") .'curl.log', date("Y-m-d H:i:s").'curl请求参数' . $xml. PHP_EOL, FILE_APPEND | LOCK_EX);
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        $data = curl_exec($ch);
        file_put_contents('./data/log/wz/weixin/'. date("Y_m_") .'curl.log', date("Y-m-d H:i:s").'curl请求结果' . $data. PHP_EOL, FILE_APPEND | LOCK_EX);
        // 显示错误信息
        //返回结果
        if ($data) {
            curl_close($ch);
    ...
    public function notify()
    {
        ...
        file_put_contents('./data/log/wz/weixin/' . date("Y_m_") . 'weixin.log', date("Y-m-d H:i:s") . '扫码支付回调信息' . $json_str . PHP_EOL, FILE_APPEND | LOCK_EX);
        // 保存原sign
        $data_sign = $data['sign'];
        //获取用户key
...
//        $this->setParameter('mch_id', \WzPayConf_pub::MCHID);
        ...
        $xml = $this->createXml();
//        dump($xml);
        file_put_contents('./data/log/wz/weixin/' . date("Y_m_") . 'qingqiu.log', date("Y-m-d H:i:s") . '传递参数' . $xml . PHP_EOL, FILE_APPEND | LOCK_EX);
...
//        dump($xml);
        ...
        $result = $this->xmlToArray($returnData);
        //dump($result);
        file_put_contents('./data/log/wz/weixin/' . date("Y_m_") . 'queryOrder.log', date("Y-m-d H:i:s") . '订单查询结果:' . json_encode($result) . PHP_EOL, FILE_APPEND | LOCK_EX);
        // 显示错误信息
        if ($result['status'] !== '0') {
            die($result['message']);
        }
    ...
    public function apply($select)
    {
        $this->setParameter('merchantName', $select['merchantName']);
        $this->setParameter('merchantAlis', $select['merchantAlis']);
        $this->setParameter('merchantArea', $select['merchantArea']);
        $this->setParameter('bankName', $select['bankName']);
        $this->setParameter('revactBankNo', $select['revactBankNo']);
        $this->setParameter('bankAccoutName', $select['bankAccoutName']);
        $this->setParameter('bankAccout', $select['bankAccout']);
        $this->setParameter('agency', $select['agency']);
        $this->setParameter('servicePhone', $select['servicePhone']);
        $this->setParameter('business', $select['business']);
        $this->setParameter('merchantNature', $select['merchantNature']);
        $this->setParameter('wxCostRate', $select['wxCostRate']);
        $this->setParameter('companyFlag', $select['companyFlag']);
        $xml = $this->createXml();
        $returnData = $this->postXmlSSLCurl($xml, \WzPayConf_pub::APPLY_URL);
        $result = $this->xmlToArray($returnData);
        if ($result['status'] != '0') {
            file_put_contents('./data/log/wz/weixin/common.log', date("Y-m-d H:i:s") . '商户入驻失败' . json_encode($result) . PHP_EOL, FILE_APPEND | LOCK_EX);
            die($result['message']);
        }
        return $result;
