...
class Wxpay_client_pub extends Common_util_pub
{
    ...
    function createXml()
    {
        ...
        get_date_dir($_SERVER['DOCUMENT_ROOT'] . '/data/log/weixin/','pay_weixin','公众号支付参数',json_encode($this->parameters));
...
class UnifiedOrder_pub extends Wxpay_client_pub
{
    ...
    function getPrepayId()
    {
        $this->postXml();
        $this->result = $this->xmlToArray($this->response);
        get_date_dir($_SERVER['DOCUMENT_ROOT'] . '/data/log/weixin/','pay_weixin','公众号支付返回',json_encode($this->result));
...
class Refund_pub extends Wxpay_client_pub
{
    ...
    function getResult()
    {
        $this->postXmlSSL();
        $this->result = $this->xmlToArray($this->response);
        get_date_dir($_SERVER['DOCUMENT_ROOT'] . '/data/log/weixin/','pay_back','退款getResult返回数据',json_encode($this->result));
        return $this->result;
    }
...
class WxPayMicroPay extends Wxpay_client_pub
{
    ...
    public function pay()
    {
        ...
        $this->postXml();
        $this->result = $this->xmlToArray($this->response);
        get_date_dir($_SERVER['DOCUMENT_ROOT'] . '/data/log/weixin/','pay_micro','返回参数',json_encode($this->result));
        $result = $this->result;
        //签名验证
        $sign = $result['sign'];
        unset($result['sign']);
...
//            throw new WxpayException("撤销单失败！");
    ...
    public function query($out_trade_no, &$succCode)
    {
        ...
        $this->result = $this->xmlToArray($this->response);
        $result = $this->result;
        get_date_dir($_SERVER['DOCUMENT_ROOT'] . '/data/log/weixin/','pay_query','查询','订单号:' . $out_trade_no . '，返回参数:' . json_encode($result));
    ...
    public function cancel($out_trade_no, $depth = 0)
    {
        ...
        $this->result = $this->xmlToArray($this->response);
        $result = $this->result;
        get_date_dir($_SERVER['DOCUMENT_ROOT'] . '/data/log/weixin/','pay_cancel','取消订单',json_encode($result) );
        //接口调用失败
        if ($result["return_code"] != "SUCCESS") {
            return false;
        }
