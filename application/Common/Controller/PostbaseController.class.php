...
class PostbaseController extends Controller
{
    ...
    protected $no_need_check_rules = array(
        ...
        "PostIndexlogout",
        "PostIndexapp_update",
        "PostIndexpos_pay_order",
        "PostIndexpos_pay_order_0",
        "PostIndexaaaa",
        "PostIndexpos_pay_order_zero",
        "PostIndexcard_coupon_check",
        "PostIndexpos_yu_pay_back",
    );
    ...
    function _initialize()
    {
        ...
        $this->token = I('token');
        //$this->token  = "9692BEB0D0952996E9DA72599F62F45CF8D00D55";
    ...
    {
        $user_info = M("post_token")->where(array("token" => $token))->getField("value");
        if (!$user_info) {
            return false;
        } else
    ...
    protected function _getSign($param, $sign)
    {
        ...
        if (is_array($param) && !empty($param) && $sign != '' && $param['timestamp'] != '') {
            ...
            //file_put_contents('./sign.log', date("Y-m-d H:i:s") . 'token=' . $this->token . '的签名' . $str . PHP_EOL, FILE_APPEND | LOCK_EX);
            return $str === $sign;
        } else {
            return false;
        }
