...
//        );
        ...
        $data = $model->field(C('DB_PREFIX') . "merchants_agent.*," . C('DB_PREFIX') . "merchants_users.user_phone," . C('DB_PREFIX') . "merchants_users.agent_id")
            ->join(' left JOIN  ' . C('DB_PREFIX') . 'merchants_users ON ' . C('DB_PREFIX') . 'merchants_users.id = ' . C('DB_PREFIX') . 'merchants_agent.uid')
//            ->page($p, C('ADMIN_PAGE_ROWS'))
            ->where($map)
            ->order('id desc')
...
//        }
    ...
    {
        $id = I("id");
        $res = M("merchants_agent")->where(array('id' => $id))->find();
        if ($res) {
            file_put_contents('./data/log/change_phone.log', date("Y-m-d H:i:s") .  '删除代理 ' . json_encode($res) . PHP_EOL, FILE_APPEND | LOCK_EX);
...
//            $this->ajaxReturn(array("states" => 'fail','referer'=>'adminIndex/index','info'=>'删除成功'));
        ...
        if (IS_POST) {
            ...
            if (isset($status)) {
                if($type=='status'){
                    ...
                    $save=array('card_auth'=>$status);
                    if($user_info = M('token')->where(array('uid' => $this->UID))->getField('value')){
                        $user_info = json_decode($user_info, true);
                        $user_info['card_auth'] = $status;
                        M('token')->where(array('uid' => $this->UID))->save(array('value' => json_encode($user_info)));
                    }
                }
                $res = M("merchants_agent")->where(array('id' => $id))->setField($save);
                if ($res !== false) {
                    $this->ajaxReturn(array('code' => '1', 'msg' => '修改成功'));
                } else {
                    $this->ajaxReturn(array('code' => '0', 'msg' => '修改失败'));
	...
	public function zsshow(){
        $id=$_GET['id'];
        $list=M('merchants_agent')->where("id='{$id}'")->find();
        $uid=$list['uid'];
        $phone=M('Merchants_users')->where("id='{$uid}'")->find();
        $this->assign('phone',$phone);
        $this->assign('list',$list);
        $this->assign('id',$id);
        $merchants_mpay_data=M('merchants_agent_into')->where(array('merchant_id'=>$id))->find();
    ...
    public function upload_into(){
        ...
        $upload->savePath  =     'zsinto/'; // 设置附件上传（子）目录
        // 上传文件
        $info   =   $upload->upload();
        if($info){
            $data['type']=1;
    ...
    public function upload_zspay(){
        ...
        $row=M('merchants_agent_into')->where(array('merchant_id'=>$merchant_id))->find();
        if($row){
            M('merchants_agent_into')->where(array('merchant_id'=>$merchant_id))->save($data);
            $this->success('保存成功!');
        }else{
            $data['merchant_id']=$merchant_id;
            M('merchants_agent_into')->add($data);
            $this->success('添加成功!');
        }
    }
