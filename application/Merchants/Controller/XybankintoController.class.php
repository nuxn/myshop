 ...
 */
class XybankintoController extends AdminbaseController
{
    protected $shopcates;
    protected $merchants;
    protected $merchants_xypay;
    ...
    function _initialize()
    {
        ...
        $this->merchants_xypay = M("merchants_xypay");
    }
    ...
    public function index()
    {
        ...
        $pay_style = I('pay_style');
        if($merchant_name){
            $map['m.merchant_name'] = array('like', "%{$merchant_name}%");
            $formget['merchantAlis'] = $merchant_name;
        }
        if($pay_style!=''){
            $map['w.pay_style'] = $pay_style;
            $formget['pay_style'] = $pay_style;
        }
        $count = $this->merchants_xypay->join('w left join ypt_merchants m on w.merchant_id=m.id')->where($map)->order('w.id desc')->count();

        $page = $this->page($count, 20);
        $xypays = $this->merchants_xypay
            ->field('w.id,w.merchant_id,w.mch_id,m.merchant_name,w.wx_code,w.ali_code,w.pay_style')
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            $check = $this->merchants_xypay->where(array('merchant_id' => $data['merchant_id']))->find();
            ...
            $res = $this->merchants_xypay->add($data);
            if($res){
                $this->redirect(U('Xybankinto/index'));
            } else{
                $this->success('未作改动');
            }
        }else{
    ...
    public function edit()
    {
        if(IS_POST){
            ...
            unset($data['id']);
            $this->merchants_xypay->where(array('id'=>$id))->save($data);
            $this->redirect(U('Xybankinto/index'));
        } else {
            $id = I('id');
            $info = $this->merchants_xypay->where(array('id' => $id))->find();
