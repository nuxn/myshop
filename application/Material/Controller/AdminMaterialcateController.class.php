...
class AdminMaterialcateController extends AdminbaseController
{
    ...
    protected $material_cate_model, $shopcates;
    const brand = 'YPT';
    ...
    private function _lists($where = array())
    {
        ...
        $this->material_cate_model
            ->alias("m")
            ->where($where);

        $count = $this->material_cate_model->count();
        ...
        $this->material_cate_model
            ...
            ->order("m.id DESC");
        $data_lists = $this->material_cate_model->select();

        foreach ($data_lists as $k => $v) {
            $name = $v['name'];
            $map['no_number'] = array('like', "%$name%");
            $num1 = $this->shopcates->where($map)->count();
            $num2 = M("material")->where(array("cate_id"=>$v['id']))->count();
            $data_lists[$k]['num1']=$num1;
            $data_lists[$k]['num2']=$num2;
            $data_lists[$k]['num'] = $num1+$num2;
        }
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            if ($this->material_cate_model->add($data)) {
                $this->success("添加成功！");
            } else {
                $this->error("添加失败！");
            }
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            $this->material_cate_model->where(array("id" => $data['id']))->save($data);
            $this->success("保存成功！");
        } else {
            $id = I("get.id", 0, 'intval');
            $info = $this->material_cate_model->where(array("id" => $id))->find();
    ...
    public function delete()
    {
        if (isset($_GET['id'])) {
            $id = I("get.id", 0, 'intval');
            if ($this->material_cate_model->where(array('id' => $id))->save(array('status' => '-1')) !== false) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
            }
        ...
        if (isset($_POST['ids'])) {
            ...
            if ($this->material_cate_model->where(array('id' => array('in', $ids)))->save(array('status' => '-1')) !== false) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
            }
