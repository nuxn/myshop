...
class PublicController extends AdminbaseController
{
    ...
    public function dologin()
    {
        ...
        //验证码
        if (!sp_check_verify_code($verrify)) {
...
//            echo $result['user_pass'] . '<br/>';
//            echo sp_password($pass) . '<br/>';
//            echo sp_password_old($pass) . '<br/>' ;
            if (!empty($result) && $result['user_type'] == 1) {
                if (sp_compare_password($pass, $result['user_pass'])) {
					...
					if($result['platform'] != 1){
						$this->error('该账号不能登陆该后台');
					}
					$uid = M('merchants_users')->where(array('user_phone'=>$result['mobile']))->getField('id');
					/*if($name != '13128898154' && $result["muid"]=='1'){
                        $result['muid']=='0';
                    }*/
                    //登入成功页面跳转
					session('muid',$uid);
