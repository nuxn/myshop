...
class WeixinbaseController extends Controller
{
    ...
    private function receiveEvent($object)
    {
        $content = "";
        switch ($object->Event) {
            case "card_pass_check"://商店审核事件
                file_put_contents('./kajuan.log', date("Y-m-d H:i:s") . '商户审核信息' . json_encode($object) . PHP_EOL, FILE_APPEND | LOCK_EX);
                $card_id = $object->CardId;
                if (!$card_id) {
                    file_put_contents('./kajuan.log', date("Y-m-d H:i:s") . '审核信息失败' . $card_id . PHP_EOL, FILE_APPEND | LOCK_EX);
                }
                break;
            case "user_get_card":// 优惠卷领取事件推送
                file_put_contents('./kajuan.log', date("Y-m-d H:i:s") . '用户领取优惠券' . json_encode($object) . PHP_EOL, FILE_APPEND | LOCK_EX);
                $card_id = $object->CardId;
                $data = array();
                ...
                file_put_contents('./kajuan.log', date("Y-m-d H:i:s") . '判断是否会员卡' . $is_screen_coupons . PHP_EOL, FILE_APPEND | LOCK_EX);
                if (!$is_screen_coupons || $is_screen_coupons == '') {
                    file_put_contents('./kajuan.log', date("Y-m-d H:i:s") . '会员卡开始' . PHP_EOL, FILE_APPEND | LOCK_EX);
            ...
            case "submit_membercard_user_info"://用户提交资料推送
                file_put_contents('./kajuan.log', date("Y-m-d H:i:s") . '会员卡用户提交资料推送' . PHP_EOL, FILE_APPEND | LOCK_EX);
