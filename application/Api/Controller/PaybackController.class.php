...
//    银行卡支付成功
    ...
    public function pay_back()
    {
        ...
        if ($pay['mode'] == 12) {
            ...
            $card = M('screen_memcard_use u')
                ...
                ->field('u.yue,u.card_code,u.entity_card_code,m.card_id,m.merchant_name,u.fromname,u.card_amount,u.card_balance')
                ->find();
            $user_yue = $card['yue'];
            ...
            $dec_card_balance = 0;
            #其他使用储值订单退款处理
            $order_info = M('order')->where(array('order_sn' => $remark))->field('card_code,user_money,integral_money')->find();
            if ($order_info) {
                if ($order_info['user_money'] > 0 && $pay['price'] > 0) {
                    $this->ajaxReturn(array("code" => "error", "msg" => "混合支付订单不能退款"));
                }
                #如果使用了储值,1.8.4版本过后储值支付和微信(支付宝)支付不能混合
                #使用了储值就不走通道退款了
                if($order_info['user_money'] > 0 && $pay['price'] == 0){
                    $card = M('screen_memcard_use u')
                        ->join('join ypt_screen_memcard m on m.id=u.memcard_id')
                        ...
                        ->field('u.yue,u.card_code,u.entity_card_code,m.card_id,m.merchant_name,u.fromname,u.card_amount,u.card_balance')
                        ->find();
                    $final_yue = $card['yue'] + $price_back;
                    $refund_price = $price_back;
                    if($order_info['user_money'] == $price_back){
                        $pay_back_all = true;
                    }
                    ...
                    if ($is_agent) {
                        #如果是代理商会员卡
                        if ($price_back != $order_info['user_money']) $this->ajaxReturn(array("code" => "error", "msg" => "该笔订单使用了异业联盟卡必须全额退款"));
                    ...
                    $this->pays->where(array('remark'=>$remark))->save(array("status" => 2, "back_status" => 1, "price_back" => $price_back));
                    $back_info = $this->add_pay_back($pay, 98, $price_back,$_POST['goods_param']);
                    $this->add_order_goods_number($remark,$_POST['goods_param']);
                    $this->reduce_cz($card,$final_yue,$remark,$refund_price,$pay_back_all);
        ...
        }elseif ($price_back == $price) {
            $pay_back_all = true;
        }
        if ($style == 1) { //现金退款
            if ($pay) $this->pays->where("remark='$remark'")->save(array("status" => "2", "price_back" => $price_back, "back_status" => 1));
            $back_info = $this->add_pay_back($pay, 99, $price_back,$_POST['goods_param']);
            if ($dec_card_balance) {
                M('merchants_users')->where(array('id' => $this->userId))->setDec('card_balance', $dec_card_balance);
                M('balance_log')->add(array('price' => -$dec_card_balance, 'ori_price' => -$dec_card_balance, 'rate_price' => 0, 'order_sn' => $remark, 'add_time' => time(), 'remark' => '退款扣除余额', 'mid' => $this->userId, 'balance' => M('merchants_users')->where(array('id' => $this->userId))->getField('balance')));
            }
            if ($card) $this->reduce_cz($card, $final_yue, $remark, $refund_price,$pay_back_all);
            $this->add_order_goods_number($remark,$_POST['goods_param']);
            $this->ajaxReturn(array("code" => "success", "msg" => "退款成功", 'back_info' => $back_info));
        } else if ($style == 2) {
            // 微信支付
            if ($pay['bank'] == 3) {
                ...
                if ($result['code'] == "success") {
                    $result['back_info'] = $this->add_pay_back($pay, 98, $price_back,$_POST['goods_param']);
                    if ($dec_card_balance) {
                        M('merchants_users')->where(array('id' => $this->userId))->setDec('card_balance', $dec_card_balance);
                        M('balance_log')->add(array('price' => -$dec_card_balance, 'ori_price' => -$dec_card_balance, 'rate_price' => 0, 'order_sn' => $remark, 'add_time' => time(), 'remark' => '退款扣除余额', 'mid' => $this->userId, 'balance' => M('merchants_users')->where(array('id' => $this->userId))->getField('balance')));
                    }
     ...
     * @param $goods
     */
    private function add_order_goods_number($remark = 0,$goods)
    {
        if (!$remark || !$goods) return;
        $order_id = $this->pays->where("remark='$remark'")->getField("order_id");
        if ($order_id && $goods) {
            $order_goods_list = json_decode($goods,true);
            #只改接收到的退货商品
            if ($order_goods_list['goods']) {
                $goods_model = M('goods');
                foreach ($order_goods_list['goods'] as &$v) {
                    //更新库存
                    if ($v['goods_id']) {
                        $goods_model->where(array("goods_id" => $v['goods_id'],'mid'=>$this->userId))->setInc('goods_number', $v['goods_num']);
                    } elseif ($v['bar_code']) {
                        $goods_model->where(array("bar_code" => $v['bar_code'],'mid'=>$this->userId))->setInc('goods_number', $v['goods_num']);
                    }
                    //更新多规格商品库存
                    if($v['sku']) {
                        M('goods_sku')->where(array('sku_id'=>$v['sku']))->setInc('quantity',$v['goods_num']);
                    }
                }
            }
        }
     ...
     * 退款成功添加到退款记录表
     * @param $pay_back 流水信息
     * @param $mode 退款类型，98=原路退款，99=现金退款
     * @param $price_back 退款金额
     * @param $goods_info 退货，退运费信息
     * @return array
     */
    protected function add_pay_back($pay_back, $mode, $price_back,$goods_info)
    {
        ...
        if ($order && $order['user_money']) {
            ...
            $ts['custom_field_value1'] = urlencode($card['yue'] + $price_back);//会员卡余额
     ...
     * 储值订单退款撤回充值的储值
     * @param $card 会员卡信息
     * @param $final_yue 用户卡内应剩余多少储值
     * @param $remark 流水号
     * @param $refund_price 退款的金额
     * @param $pay_back_all 是否是全额退款，全额退款退积分
     */
    private function reduce_cz($card, $final_yue, $remark,$refund_price,$pay_back_all)
    {
        M()->startTrans();
        ...
        if ($memcard_use_where) {
            M('screen_memcard_use')->where($memcard_use_where)->save(array('yue'=> $final_yue));
        }
        #是否是全额退款，全额退款退积分
        if($pay_back_all) {
            $integral = M('order')->where(array('order_sn'=>$remark))->getField('integral');
            M('screen_memcard_use')->where($memcard_use_where)->save(array('card_amount'=>$card['card_amount']+$integral,'card_balance'=>$card['card_balance']+$integral));
            $ts['record_bonus'] = urlencode("退款退还积分");
