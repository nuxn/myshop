...

{
    ...
    //卡券核销1.8.7版本,应付金额 = (订单金额-优惠券-积分) X 会员卡折扣
    public function cou_to_men1()
    {
        $this->path = $_SERVER['DOCUMENT_ROOT'] . '/data/log/Api/';
        $code = I("code");
        $price = I("price");
        ...
        $agent_id = M('merchants_users')->where(array('id' => $mch_uid))->getField('agent_id');
        if ($agent_id == '0') $agent_id = '-1';
        get_date_dir($this->path, 'App_cou_to_men1', '获取参数', json_encode($_POST));
        //优惠券
        if ($data = M("screen_user_coupons")->where(array("usercard" => $code, "status" => 1))->find()) {
            $map = array("c.id" => $data['coupon_id'], 'c.card_type' => 'GENERAL_COUPON');
            $res = M('screen_coupons')->alias('c')
            ...
            if ($res['memid']) {
                ...
                $card = M('screen_memcard')->alias('m')
                    ->join('join ypt_screen_memcard_use u on m.id=u.memcard_id')
                    ->where($map)
                    ->field('u.card_amount,u.memid,u.yue,u.level,u.card_id,u.card_balance,
                            m.id,m.max_reduce_bonus,m.credits_set,m.integral_dikou,m.max_reduce_bonus,balance_set,
                            m.credits_use,m.credits_discount,m.discount_set,m.discount,m.mid,m.level_set')
                    ->find();
                if ($card) {
                    $card['card_code'] = $card['card_code'] ?: $card['entity_card_code'];
                }
                #查看是否这个会员是否有代理商会员卡
                if (!$card && $agent_id > 0) {
                    $map['m.mid'] = $agent_id;
            ...
            $integral_money = '0';//积分对应金额
            $integral = '0';//使用的积分值
            $yue = '0';//可使用的储值
            #如果有会员卡
            if ($card) {
                #2.算积分
                if ($card['integral_dikou'] == 1) {
                    //计算可使用的积分能抵扣的金额
                    if ($card['card_balance'] < $card['max_reduce_bonus']) {
                        #如果剩余积分小于最大可用积分，(剩余积分/使用XX积分)*对应XX积分抵扣的金额
                        $p = floor($card['card_balance'] / $card['credits_use']) * $card['credits_discount'];
                    } else {
                        #如果剩余积分大于最大可用积分，(最大可使用积分/使用XX积分)*对应XX积分抵扣的金额
                        $p = floor($card['max_reduce_bonus'] / $card['credits_use']) * $card['credits_discount'];
                    }
                    //计算积分能抵扣的金额，和使用了多少积分
                    if ($p < $pay_price) {
                        #如果计算的金额小于应支付金额
                        $integral_money = $p;
                        $integral = $p / $res['credits_discount'] * $res['credits_use'];
                    } else {
                        #如果计算的金额大于应支付金额
                        $integral = floor($pay_price / $res['credits_discount']) * $res['credits_use'];
                        $integral_money = ($data['jifen_use'] / $res['credits_use']) * $res['credits_discount'];
                    }
                }
                $pay_price -= $integral_money;
                #3.算折扣
                if ($res['level_set'] == '1') {
                    $discount = M('screen_memcard_level')->where(array('c_id' => $res['id'], 'level' => $res['level']))->getField('level_discount') * 0.1;
                } elseif ($res['discount_set'] == 0 || $res['discount'] == 0 || !$res['discount']) {
                    $discount = '1';
                } else {
                    $discount = $res['discount'] * 0.1;
                }
                $discount_price = $pay_price * $discount;

                //4.储值
                //判断储值抵扣开关是否打开
                if($res['balance_set']==1){
                    $yue = $res['yue'];
                }
            }
            $result = array(
                'card_code' => strval($card['card_code']),//会员卡号
                'dikoufen' => strval($integral),//使用的积分
                'dikoujin' => strval($integral_money),//使用积分对应的金额
                'coupon_code' => strval($code),//优惠券号码
                'coupon_price' => strval($res['de_price']),//优惠券抵扣金额
                'discount' => strval($discount),//折扣
                'discount_price' => strval($discount_price),//折扣对应的金额
                'total_de_price' => strval($integral_money + $res['de_price'] + $discount_price),//总优惠金额
                'yue' => strval($yue),//使用储值的金额
                'memid' => strval($memId),
                'code_type' => '1'//请求接口的卡号类型，1优惠券，2会员卡
            );
            $this->ajaxReturn(array("code" => "success", "msg" => "成功", "data" => $result));
            //会员卡
        } elseif ($d = M("screen_memcard_use")->where(array('status' => 1, 'card_code|entity_card_code' => $code))->find()) {
            $map['u.card_code'] = $code;
            $map['u.entity_card_code'] = $code;
            $map['_logic'] = 'OR';
            $res = M('screen_memcard')->alias('m')
                ...
                ->field('u.card_amount,u.memid,u.yue,u.level,u.card_id,u.card_balance,m.id,m.max_reduce_bonus,m.credits_set,m.integral_dikou,m.max_reduce_bonus,m.credits_use,m.credits_discount,m.discount_set,m.discount,m.mid,m.level_set')
                ->find();
            get_date_dir($this->path, 'App_cou_to_men1', '获取会员卡', M()->_sql());
            if ($res['mid'] != $mch_uid && $res['mid'] != $agent_id) $this->ajaxReturn(array("code" => "error", "msg" => "该会员卡不能在本店使用"));
            if ($res['mid'] == $agent_id) {
                $use_merchants = M('screen_memcard')->alias('m')
                    ->join('join ypt_screen_memcard_use u on m.id=u.memcard_id')
            ...
            //1.算优惠券
            $where = array('c.card_type' => 'GENERAL_COUPON', 'm.uid' => $mch_uid, 'mem.id' => $res['memid'], 'uc.status' => 1, 'c.total_price' => array('ELT', $price), 'c.begin_timestamp' => array('ELT', time()), 'c.end_timestamp' => array('EGT', time()));
            $coupon = M('screen_user_coupons')->alias('uc')
                ->join('join ypt_screen_coupons c on uc.coupon_id=c.id')
                ->join('join ypt_screen_mem mem on mem.unionid=uc.unionid')
                ->join('left join ypt_merchants m on m.id=c.mid')
            ...
            $pay_price *= $discount;
            $discount_price = $price - $pay_price;
            $result = array(
                'card_code' => strval($code),
                'dikoufen' => strval($integral),
                'dikoujin' => strval($integral_money),
                'coupon_code' => strval($coupon['usercard']),
                'coupon_price' => strval($coupon['de_price']),
                'discount' => strval($discount),
                'discount_price' => strval($discount_price),
                'total_de_price' => strval($data['dikoujin'] + $coupon['de_price'] + $discount_price),
                'yue' => strval($res['yue']),
                'code_type' => '2',
                'memid' => strval($res['memid'])
            );
            $this->ajaxReturn(array("code" => "success", "msg" => "成功", "data" => $result));
        } else {
            get_date_dir($this->path, 'App_cou_to_men1', '获取会员卡', M()->_sql());
	...
	public function zk_detail2()
    {
        ...
        $card_list = array();
        //本店卡
        if($res) $card_list[] = $this->card_calc2($res,$coupon_code,$price,'0');
        //联名卡
        if($res_agent) $card_list[] = $this->card_calc2($res_agent,$coupon_code,$price,'1');
        if($memid){
            $mem = M('screen_mem')->where("id=$memid")->field('memimg,memphone')->find();
            $coupon = M('screen_user_coupons')->alias('uc')
                ->join('join ypt_screen_coupons c on uc.coupon_id=c.id')
    ...
    //1.8.7计算会员卡
    public function card_calc2($res,$coupon_code,$price,$is_agent){
        $pay_price = $price;
        //1.算优惠券
        if($coupon_code){
            $coupon_de_price = M('screen_user_coupons')->alias('uc')
        ...
        if($res['integral_dikou']==0){
            ...
            if($p<$pay_price){
                ...
                $card['dikoufen'] = floor($pay_price/$res['credits_discount'])*$res['credits_use'];
                $card['dikoujin'] = ($card['dikoufen']/$res['credits_use'])*$res['credits_discount'];
            }
            $pay_price -= $card['dikoujin'];
        }
        //3.算折扣
        if($res['level_set']=='1'){
            $card = M('screen_memcard_level')->where(array('c_id'=>$res['id'],'level'=>$res['level']))->field('level_name,level_discount*0.1 as discount')->find();
