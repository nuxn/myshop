...
//    银行卡支付成功
    ...
    public function pay_back()
    {
        ...
        if ($pay['mode'] == 12) {
            ...
            $dec_card_balance = 0;
            if ($order_info) {
                if($goods_info = M('order_goods')->where(array('order_id'=>$order_info['order_id']))->select()){
                    $goods = explode(';',I('goods_param'));
                    $goodsParam = explode(':',$goods);
                }
                #如果使用了储值,1.8.4版本过后储值支付和微信(支付宝)支付不能混合，使用了储值就不走下面通道退款了
                if($order_info['user_money'] > 0){
                    ...
                    if($pay['price'] == 0) {
                        //$this->add_order_goods_number($remark);
                        $this->reduce_cz($card,$final_yue,$remark,$refund_price);
                        $this->ajaxReturn(array("code" => "success", "msg" => "退款成功", 'back_info' => $back_info));
                    }
                }
            }
